...
if /i "%~1"=="test" (
    ...
    if errorlevel 1 exit /b 1
    gcc -std=c17 -I. -Wall -Wextra -Wpedantic -Werror -O0 -g tests\test_image_wic.c chat\shell\image_wic.c chat\core\image_policy.c -o build\test_image_wic.exe -lwindowscodecs -loleaut32 -lole32 -Wl,--wrap=malloc -Wl,--wrap=free
    if errorlevel 1 exit /b 1
    build\test_image_wic.exe
    if errorlevel 1 exit /b 1
    gcc -std=c17 -I. -Wall -Wextra -Wpedantic -Werror -O0 -g tests\test_openrouter.c chat\generation\context.c chat\generation\provider_routing.c chat\generation\completion_request.c chat\core\chat.c chat\json.c chat\generation\sse.c -o build\test_openrouter.exe -lwinhttp -Wl,--wrap=free
    if errorlevel 1 exit /b 1
    ...
    if errorlevel 1 exit /b 1
    windres tests\test_chat_host.rc -O coff -o build\test_chat_host.res
    if errorlevel 1 exit /b 1
    gcc -std=c17 -I. -Wall -Wextra -Wpedantic -Werror -O0 -g tests\test_chat_host.c chat\generation\context.c chat\generation\provider_routing.c chat\core\search.c chat\core\chat.c chat\shell\chat_ui.c chat\transcript\transcript_policy.c chat\transcript\transcript_win32.c chat\persistence\storage.c chat\persistence\saver.c chat\persistence\attachments.c chat\shell\actions_win32.c chat\transcript\rich_text_win32.c chat\transcript\markdown.c chat\transcript\table_layout.c chat\json.c chat\export\export.c chat\import\import.c chat\generation\sse.c chat\generation\completion_request.c chat\generation\completion_winhttp.c chat\models\model_catalog.c chat\models\model_catalog_winhttp.c chat\shell\palette.c chat\shell\palette_win32.c ui\ui.c ui\theme.c ui\paint.c platform\renderer.c platform\accessibility.c platform\dark_mode_win32.c chat\core\commands.c build\test_chat_host.res -o build\test_chat_host.exe -Wl,--wrap=completion_request -Wl,--wrap=model_catalog_request -Wl,--wrap=palette_popup_pump -Wl,--wrap=storage_save -Wl,--wrap=malloc -Wl,--wrap=realloc -Wl,--wrap=free -Wl,--wrap=rich_text_create_block -Wl,--wrap=rich_text_create_viewport -Wl,--wrap=chat_copy_text -Wl,--wrap=chat_edit_dialog -Wl,--wrap=chat_save_dialog -Wl,--wrap=chat_open_dialog -Wl,--wrap=chat_read_file_utf8 -Wl,--wrap=chat_write_file_utf8 -Wl,--wrap=chat_export_timestamp -Wl,--wrap=chat_shell_notify -Wl,--wrap=chat_foreground_window -Wl,--wrap=chat_set_foreground -Wl,--wrap=chat_ime_active -ld2d1 -ldwrite -ldwmapi -luiautomationcore -loleaut32 -lole32 -lgdi32 -lshell32 -lwinhttp -lcomdlg32 -limm32 -lcomctl32
    if errorlevel 1 exit /b 1
