...
if /i "%~1"=="test" (
    ...
    if errorlevel 1 exit /b 1
    gcc -std=c17 -I. -Wall -Wextra -Wpedantic -Werror -O0 -g tests\test_chat_host.c chat\generation\context.c chat\generation\provider_routing.c chat\core\search.c chat\core\chat.c chat\shell\chat_ui.c chat\transcript\transcript_policy.c chat\transcript\transcript_win32.c chat\persistence\storage.c chat\persistence\saver.c chat\shell\actions_win32.c chat\transcript\rich_text_win32.c chat\transcript\markdown.c chat\transcript\table_layout.c chat\json.c chat\export\export.c chat\generation\sse.c chat\generation\completion_request.c chat\generation\completion_winhttp.c chat\models\model_catalog.c chat\models\model_catalog_winhttp.c chat\shell\palette.c chat\shell\palette_win32.c ui\ui.c ui\theme.c ui\paint.c platform\renderer.c platform\accessibility.c chat\core\commands.c -o build\test_chat_host.exe -Wl,--wrap=completion_request -Wl,--wrap=model_catalog_request -Wl,--wrap=palette_popup_pump -Wl,--wrap=storage_save -Wl,--wrap=malloc -Wl,--wrap=realloc -Wl,--wrap=rich_text_create_block -Wl,--wrap=rich_text_create_viewport -Wl,--wrap=chat_copy_text -Wl,--wrap=chat_edit_dialog -Wl,--wrap=chat_save_dialog -Wl,--wrap=chat_write_file_utf8 -Wl,--wrap=chat_export_timestamp -ld2d1 -ldwrite -ldwmapi -luiautomationcore -loleaut32 -lole32 -lgdi32 -lshell32 -lwinhttp -lcomdlg32
    if errorlevel 1 exit /b 1
