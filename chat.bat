...
if /i "%~1"=="test" (
    ...
    if errorlevel 1 exit /b 1
    gcc -std=c17 -Wall -Wextra -Wpedantic -Werror -O0 -g tests\test_chat_host.c chat\context.c chat\provider_routing.c chat\search.c chat\chat.c chat\chat_ui.c chat\transcript_policy.c chat\transcript_win32.c chat\storage.c chat\saver.c chat\actions_win32.c chat\rich_text_win32.c chat\markdown.c chat\table_layout.c chat\json.c chat\sse.c chat\completion_request.c chat\completion_winhttp.c chat\model_catalog.c chat\model_catalog_winhttp.c chat\model_picker_win32.c ui\ui.c ui\theme.c ui\paint.c platform\renderer.c platform\accessibility.c chat\commands.c -o build\test_chat_host.exe -Wl,--wrap=completion_request -Wl,--wrap=model_catalog_request -Wl,--wrap=model_picker_pump -Wl,--wrap=storage_save -Wl,--wrap=malloc -Wl,--wrap=realloc -Wl,--wrap=rich_text_create_block -Wl,--wrap=rich_text_create_viewport -ld2d1 -ldwrite -ldwmapi -luiautomationcore -loleaut32 -lole32 -lgdi32 -lshell32 -lwinhttp
    if errorlevel 1 exit /b 1
